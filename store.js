...
function bind(conn) { 
    ...
    conn.ev.on('chats.set', async ({ chats }) => { 
        try { 
            for (let { id, name, readOnly } of chats) { 
                ...
                console.log(`Chat updated: ${id}`); // Logging
            } 
        } catch (e) { 
            console.error(e); 
        } 
    ...
    conn.ev.on('group-participants.update', async ({ id, participants, action }) => { 
        ...
        console.log(`Group participants updated: ${id}, Action: ${action}`); // Logging
    }); 

    // Aggiunta di un metodo per cercare contatti
    conn.searchContacts = (searchTerm) => {
        return Object.values(conn.chats).filter(contact => 
            contact.name && contact.name.toLowerCase().includes(searchTerm.toLowerCase())
        );
    };

    // Aggiunta di metodi per gestire i membri del gruppo
    conn.addGroupMember = async (groupId, memberIds) => {
        try {
            await conn.groupAdd(groupId, memberIds);
    ...
    conn.removeGroupMember = async (groupId, memberIds) => {
        try {
            await conn.groupRemove(groupId, memberIds);
            console.log(`Members removed from group ${groupId}: ${memberIds}`);
        } catch (error) {
            console.error(`Error removing members from group ${groupId}: ${error}`);
        }
    };
    ...
    conn.ev.on('groups.update', async (groupsUpdates) => { 
        try { 
            for (const update of groupsUpdates) { 
                ...
                console.log(`Group update pushed to DB: ${id}`); // Logging
            } 
        } catch (e) { 
            console.error(e); 
        } 
    ...
    conn.ev.on('chats.upsert', (chatsUpsert) => { 
        try { 
            ...
            console.log(`Chat upserted: ${id}`); // Logging
        } catch (e) { 
            console.error(e); 
        } 
    }); 
    ...
    conn.ev.on('presence.update', async ({ id, presences }) => { 
        try { 
            ...
            console.log(`Presence updated: ${_sender} is now ${presence}`); // Logging
        } catch (e) { 
            console.error(e); 
        } 
    }); 
